...
# Handoff 판단 노드가 추가되었습니다. \
# [버전1] 사역자 대화 모범 데이터셋만 사용합니다. \
# Upstage API를 사용합니다. 
...
print("Chroma DB Path:", persist_directory)
        ...
        # 3. CSV 파일 처리
            ...
            try:
                ...
                if 'Question' in df.columns and 'Answer' in df.columns:
                    ...
                    print(f"  [Warning] {file_path}에 'Question' 또는 'Answer' 컬럼이 없습니다.")
...
if os.path.exists(persist_directory) and len(os.listdir(persist_directory)) > 0:
    ...
    # 기존 DB를 로드할 때도 embedding_function을 꼭 넣어줘야 검색이 가능
    chroma_db = Chroma(
    ...
    # from_documents를 사용하면 persist_directory가 지정된 경우 자동 저장
...
llm_compressor = ChatUpstage(model="solar-pro", temperature=0.5)
...
# 3. 실제 Handoff 메시지를 보내는 노드
def node_handoff_responser(state: State) -> State:
    print(">> HANDOFF MESSAGE GENERATION")
    
    # 템플릿 메시지 -> 이후 번역 노드에서 페르시아어로 바뀜
    handoff_msg = (
    ...
    state["generation"] = handoff_msg # 여기서 바로 번역으로 넘기기
    return append_step_log(state, "node_handoff_responser", {"generation": handoff_msg})


# 4. 라우팅 로직 (조건부 엣지에서 사용)
def route_logic_with_handoff(state: State):
    # 1차 라우팅 결과 가져오기
    route = state["route"]
    ...
    if route == "rag":
        ...
        if decision == "handoff":
            print("   [Routing] 사람 연결(Handoff) 노드로 이동")
            return "node_handoff_responser"
        else:
            print("   [Routing] 일반 검색(RAG) 노드로 이동")
            return "node_rag_responser"
            
    elif route == "default":
        print("   [Routing] 일상 대화(Default) 노드로 이동")
        return "node_default_responser"
    else:
        print("   [Routing] 차단(Block) 노드로 이동")
...
graph = (
    ...
    .add_node("node_handoff_responser", node_handoff_responser)
    ...
    # 조건부 엣지: route_query -> (판단 로직) -> 각 노드
    .add_conditional_edges(
        "route_query",
        route_logic_with_handoff, 
        {
            "node_handoff_responser": "node_handoff_responser", # 사람 연결
            "node_rag_responser": "node_rag_responser",         # RAG 검색
            "node_default_responser": "node_default_responser", # 일상 대화
            "node_block_responser": "node_block_responser",     # 차단
        }
    )
    
    # Handoff 메시지 생성 후 -> 번역 -> 종료
    .add_edge("node_handoff_responser", "translate_english_to_persian")
